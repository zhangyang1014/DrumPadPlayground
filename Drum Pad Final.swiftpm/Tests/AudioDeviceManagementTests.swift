...
struct AudioDeviceManagementTests {
    ...
    func testInvalidDeviceSelection() async throws {
        ...
        #expect(deviceManager.currentDevice == originalDevice, "Should not change device for invalid ID")
    }
    ...
    func testRecommendedCompensationCalculation() async throws {
        ...
        let bluetoothDevice = AudioDevice(id: "bt", name: "BT", isBluetooth: true, deviceType: .bluetoothA2DP)
        let wiredDevice = AudioDevice(id: "wired", name: "Wired", isBluetooth: false, deviceType: .headphones)
        let builtInDevice = AudioDevice(id: "builtin", name: "Built-in", isBluetooth: false, deviceType: .builtInSpeaker)
...
private class MockAudioDeviceManager {
    ...
    init() {
        ...
        availableDevices = [
            AudioDevice(id: "builtin-speaker", name: "Built-in Speakers", isBluetooth: false, deviceType: .builtInSpeaker),
            AudioDevice(id: "builtin-receiver", name: "Built-in Receiver", isBluetooth: false, deviceType: .builtInReceiver)
        ]
    ...
    func simulateDeviceConnection(_ device: AudioDevice) {
...
private class MockAudioLatencyManager {
    ...
    func handleDeviceChange(_ device: AudioDevice) {
