...
struct MIDITestGenerators {
    ...
    static func generateMIDIDeviceInfo() -> MIDIDeviceInfo {
        ...
        let connectionTypes: [MIDIConnectionType] = [.usb, .bluetooth, .network]
        
        let randomName = deviceNames.randomElement()!
        let randomManufacturer = manufacturers.randomElement()!
        let randomConnectionType = connectionTypes.randomElement()!
    ...
    static func generateMIDIMapping(isComplete: Bool = true) -> MIDIMapping {
        ...
        if isComplete {
            // Generate complete mapping with at least 8 drum pads
            for i in 0..<max(8, Int.random(in: 8...drumPads.count)) {
                let drumPad = drumPads[i]
                let midiNote = Int.random(in: 24...127) // Valid MIDI note range
                mappings[drumPad] = midiNote
            }
        } else {
            // Generate incomplete mapping with fewer than 8 drum pads
            let count = Int.random(in: 1..<8)
            for i in 0..<count {
                let drumPad = drumPads[i]
                let midiNote = Int.random(in: 24...127)
                mappings[drumPad] = midiNote
            }
        }
...
struct MIDIConnectionPropertyTests {
    ...
    func testMIDIMappingValidationConsistency(mapping: MIDIMapping) async throws {
        ...
        // Test that mapping persistence works
        conductor.loadSavedMIDIMapping()
        // Note: In a real test environment, we'd need to mock UserDefaults
