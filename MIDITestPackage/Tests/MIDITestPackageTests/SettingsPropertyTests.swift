...
private enum TestColor: Equatable {
    case red, blue, green, yellow, orange, purple, white
}
...
private class MockSettingsManager {
    ...
    var streakFlashEnabled: Bool = true
    var audioLatencyCompensation: Double = 0.0
    var selectedAudioDevice: String = ""
    var dailyGoalMinutes: Int = 5
    var metronomeVolume: Double = 0.7
    var metronomeSound: String = "click"
    
    func validateSettings() -> [MockSettingsValidationError] {
        var errors: [MockSettingsValidationError] = []
    ...
    func exportSettings() -> MockSettingsExport {
        return MockSettingsExport(
    ...
    func importSettings(_ settingsExport: MockSettingsExport) throws {
        // Validate imported settings
        let tempManager = MockSettingsManager()
        ...
        if !validationErrors.isEmpty {
            throw MockSettingsImportError.validationFailed(validationErrors)
        }
...
private struct MockSettingsExport {
    let highContrastMode: Bool
    let streakFlashEnabled: Bool
    let audioLatencyCompensation: Double
    let selectedAudioDevice: String
...
private enum MockSettingsValidationError: Error {
    case invalidDailyGoal
    case invalidLatencyCompensation
    case invalidMetronomeVolume
}

private enum MockSettingsImportError: Error {
    case validationFailed([MockSettingsValidationError])
}

// MARK: - Test Helpers

private func createTestSettingsManager() -> MockSettingsManager {
