...
struct ScoreEnginePropertyTests {
    ...
    func testTimingJudgmentAccuracy() async throws {
        for _ in 0..<100 {
            let scoreEngine = MockScoreEngine()
    ...
    func testScoreCalculationConsistency() async throws {
        for _ in 0..<100 {
            ...
            // First calculation
            let scoreEngine1 = MockScoreEngine()
            ...
            let scoreEngine2 = MockScoreEngine()
    ...
    func testErrorPenaltyConsistency() async throws {
        for _ in 0..<100 {
            ...
            // Calculate expected penalty using the same logic as the implementation
            // The penalty is applied to the base score, not the real-time score
            let totalTargets = Float(targetEvents.count)
            let perfectHits = 3 // We added 3 perfect hits
            
            // Calculate base score - perfect hits have score multiplier 1.0
            // But we need to account for streak bonuses on individual hits
            // Since we have 3 consecutive perfect hits, the streak will be 1, 2, 3
            // Streak bonus applies when streak >= 4, so no individual hit bonuses
            let baseScore = (Float(perfectHits) * 1.0) / totalTargets * 100.0
            
            // Apply extra penalty
            let extraPenaltyMultiplier = 1.0 - (Float(extraHitCount) * profile.extraPenalty)
            let scoreAfterPenalty = baseScore * max(0.0, extraPenaltyMultiplier)
            
            // Apply final streak bonus (maxStreak will be 3)
            let streakBonus = Float(3) * profile.streakBonus * 100.0
            let expectedScoreAfterPenalty = max(0, min(100, scoreAfterPenalty + streakBonus))
            
            // Allow for some calculation variance due to floating point precision
            #expect(abs(result.totalScore - expectedScoreAfterPenalty) < 1.0, 
                   "Score after penalty should be consistent: expected \(expectedScoreAfterPenalty), actual \(result.totalScore)")
        }
    }
}
